...
\subsection{Assets}
As mentioned in Section \todo{referer tidligere afsnit} in this case we have a system with several servers on DTU, logging their events to a central logging server. This is done with the intent that by examining the central log file, it will be possible to find signs of a compromised server.\\
...
From the identified assets the hardware assets and the software assets could be replaced by new stuff, which would cost money. However the intellectual properties, documentation and data are however would be much harder to replace, since they are unique and not an off the shelf asset.
